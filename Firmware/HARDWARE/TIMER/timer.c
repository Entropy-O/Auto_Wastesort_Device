...
//1、更新中断部分
	...
	{	  
		...
		{	    
			...
			{
				if((TIM3CH1_CAPTURE_STA&0X3F)==0X3F)//高电平太长了111111
				{
	...
	if (TIM_GetITStatus(TIM3, TIM_IT_CC1) != RESET)//捕获1发生捕获事件
		{	
			...
			{	  			
				TIM3CH1_CAPTURE_STA|=0X80;		//标记捕获完成
				TIM3CH1_CAPTURE_VAL=TIM_GetCapture1(TIM3);//将捕获寄存器的值记录在TIM3CH1_CAPTURE_VAL中
		   		TIM_OC1PolarityConfig(TIM3,TIM_ICPolarity_Rising); //CC1P=0 设置为上升沿捕获
			}else  								//还未开始,第一次捕获上升沿
			{
				TIM3CH1_CAPTURE_STA=0;			//清空
				TIM3CH1_CAPTURE_VAL=0;
	 			TIM_SetCounter(TIM3,0);//初始化捕获值
